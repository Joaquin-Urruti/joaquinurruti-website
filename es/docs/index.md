---
title: Sobre mí
description: Joaquín Urruti — Ingeniero en IA & GIS especializado en ciencia de datos geoespaciales, agricultura de precisión, teledetección y automatización de procesos con IA para agronegocios y operaciones agrícolas.
keywords: Joaquín Urruti, Ingeniero IA & GIS, consultor GIS, ciencia de datos geoespaciales, agricultura de precisión, Google Earth Engine, PostGIS, Python, teledetección, tecnología agrícola, automatización de procesos, consultoría IA, análisis de datos espaciales, agronegocios, automatización GIS, índices de vegetación, imágenes satelitales, seguro paramétrico, optimización logística, OSRM, mapeo de cumplimiento regulatorio, automatización de flujos de trabajo, QGIS
author: Joaquín Urruti
canonical_url: https://www.joaquinurruti.com/es/
---

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Person",
  "name": "Joaquín Urruti",
  "url": "https://www.joaquinurruti.com/",
  "image": "https://www.joaquinurruti.com/assets/landing_image.jpg",
  "sameAs": [
    "https://www.youtube.com/@joaquinurruti",
    "https://www.linkedin.com/in/jurruti",
    "https://github.com/Joaquin-Urruti"
  ],
  "jobTitle": "AI & GIS Engineer",
  "worksFor": {
    "@type": "Organization",
    "name": "joaquinurruti",
    "url": "https://www.joaquinurruti.com/"
  },
}
</script>

<div class="hero-section grid-container" markdown>

<div class="text-intro-grid" markdown>

# Transformá tus datos e IA en tu ventaja competitiva

<span style="display:block; line-height:1.1; margin-bottom:0.1em; font-size:1.3em;">Tengo más de 15 años trabajando con sistemas GIS y Teledetección, y más de 5 años implementando soluciones de IA y datos</span>

- ¿Te cuesta mantenerte al día con el ritmo acelerado de la innovación en IA?

- ¿Necesitás ayuda para traducir el hype de la IA en resultados de negocio reales?

- ¿Necesitás alguien que entienda tanto la perspectiva técnica como la del negocio?

- ¿Tenés muchos datos pero no les estás sacando provecho?



[Agendá una llamada gratuita :material-arrow-top-right:](https://calendly.com/joaquin-urruti/consultation-30min){ .md-button .md-button--primary }

</div>

<div class="profile-image-grid" markdown>

![Joaquín Urruti, Ingeniero y Consultor en IA & GIS](./assets/landing_image.jpeg){ .profile-image alt="Retrato de Joaquín Urruti, Ingeniero y Consultor en IA & GIS" }

</div>

</div>

## Sobre mí

¡Hola! ¡Soy Joaquín!

Soy consultor de datos geoespaciales, [GIS](#GIS "Sistemas de Información Geográfica") e [IA](#AI "Inteligencia Artificial") de Argentina. Trabajo con empresas agrícolas, start-ups y organizaciones técnicas que enfrentan el desafío de traducir capacidades de datos e innovación en IA en eficiencia operativa y ventaja competitiva.

Recientemente lideré el departamento de Innovación y Desarrollo en Espartina S.A., una empresa argentina de producción agrícola con más de 2.000 lotes y 150.000 hectáreas de tierra, impulsando su transformación hacia un modelo de máxima eficiencia, apalancado en la tecnología, los datos y la inteligencia artificial, para crear un impacto significativo en las decisiones, los procesos y las personas. Automatizamos el análisis de nuevos campos para arrendamiento, escalamos las zonas de manejo de precisión de 50.000 a 150.000 hectáreas, logramos procesar el 100% de los datos de monitores de rendimiento, automatizamos el 100% del procesamiento de ensayos de campo e implementamos automatización impulsada por IA para interpretar los resultados del análisis estadístico de esos ensayos.

<div class="about-image-float" markdown>

![Joaquín Urruti](assets/ajedrez-frente-color.jpg){ .about-profile alt="Portrait of Joaquín Urruti, AI & GIS Engineer Advisor" }

</div>

Antes de eso, lideré la división GIS en Espartina, desarrollando herramientas y automatizando flujos de trabajo para gestionar y analizar datos espaciales y productivos. Previamente, trabajé como analista comercial de grandes cuentas en S4 Agtech, una empresa con sede en Buenos Aires especializada en seguros paramétricos de sequía para agricultura. En esa posición, combiné responsabilidades comerciales y analíticas, ayudando a los clientes a entender el riesgo de su cartera a través de datos satelitales e índices de vegetación, y explicando cómo la cobertura de seguros paramétricos podía minimizar esos riesgos. Trabajamos directamente con reaseguradoras como Munich Re en Argentina, Uruguay y Brasil, traduciendo análisis geoespaciales complejos en estrategias de gestión de riesgo accionables.

Con una sólida formación en GIS, programación en Python, Google Earth Engine y desarrollo de bases de datos Postgres/PostGIS, diseño e implemento estrategias de innovación que integran ciencia de datos geoespaciales, teledetección, automatización de procesos y optimización impulsada por IA. Mi enfoque siempre está en la eficiencia, escalabilidad y asegurar que las soluciones sean verdaderamente adoptadas por los equipos que las utilizan.

Soy Ingeniero Agrónomo de la Universidad de Buenos Aires con una Maestría en GIS Open Source y estudios de posgrado en Agronegocios, Ciencia de Datos y Blockchain.

Más allá de la agricultura y la tecnología, he explorado las aplicaciones de blockchain y criptomonedas en fintech y ag-tech, y mantengo un fuerte lado creativo a través de la música. Estudié trompeta de jazz durante varios años y me presenté como solista en diferentes big bands profesionales y combos pequeños de jazz. También toco el piano desde los seis años, enfocándome en repertorio clásico, romántico e impresionista — un viaje artístico que ha moldeado profundamente mi disciplina, sensibilidad y creatividad.


## ¿Por qué trabajar conmigo?

Esto es lo que me diferencia y cómo puedo ayudar a generar valor para tu negocio:

<div class="grid cards" markdown>

-   :fontawesome-solid-building-user:{ .lg .middle } Experiencia empresarial comprobada

    ---

    Vengo de una familia de productores agropecuarios, tengo formación agronómica y una carrera construida alrededor de operaciones agrícolas. Traduzco las necesidades del negocio en soluciones claras y accionables, haciendo que el puente entre "lo que el campo necesita" y "lo que construimos" sea rápido, natural y sin fricciones.

-   :material-youtube:{ .lg .middle } Educador y comunicador

    ---

    Mi experiencia como creador de contenido y educador significa que puedo desglosar conceptos técnicos complejos en insights claros y accionables. Siempre vas a entender el 'por qué' detrás de las decisiones técnicas y vas a recibir actualizaciones claras del progreso.

-   :material-school:{ .lg .middle } Experto de la industria

    ---

    Con más de 15 años en datos geoespaciales y teledetección, ayudo a los equipos a convertir realidades espaciales y agronómicas complejas en sistemas confiables y accionables — basándome en experiencia práctica en agricultura, seguros, ONGs de investigación, valuación/división de tierras y operaciones a gran escala.

-   :material-rocket:{ .lg .middle } Implementación rápida

    ---

    Me especializo en desarrollo e implementación rápida de soluciones de IA. Usando herramientas modernas y frameworks probados, puedo ayudarte a pasar del concepto a la producción más rápido, dándote una ventaja competitiva en el mercado acelerado de hoy.

</div>

## Lo que dicen mis clientes anteriores sobre mi trabajo

<div class="grid cards testimonials" markdown>

-   :material-format-quote-open:{ .lg .middle } Gabriel Vázquez Amábile

    Ingeniero Agrónomo (UBA), Ph.D. y Master of Science (Purdue University), Administrador de Campos y Productor, Consultor Ambiental en IPCC, PUMA, F.T di Tella, Banco Mundial, FAO, AACREA e INTA.

    ---

    "Tengo el placer de recomendar a Joaquín Urruti como un profesional altamente calificado. Como Ingeniero Agrónomo, lidera consistentemente en la adopción de herramientas avanzadas, tecnologías y enfoques analíticos para abordar la variabilidad espacial y temporal en sistemas agrícolas, entregando insights valiosos basados en datos para los tomadores de decisiones.

    Más allá de su experiencia técnica, Joaquín es un profesional confiable y comprometido que trabaja excepcionalmente bien en equipos. Su enfoque proactivo, sentido de responsabilidad y capacidad para generar sinergia lo convierten en un contribuidor valioso para cualquier entorno colaborativo.

    Recomiendo encarecidamente a Joaquín a cualquier organización o proyecto que busque experiencia de alto nivel en análisis geoespacial aplicado a la producción agrícola y evaluación del uso del suelo."


-   :material-format-quote-open:{ .lg .middle } Felipe Harrison

    Co-Fundador en AgAnalyst Ltd.

    ---

    "He tenido el placer de trabajar con Joaquín durante los últimos dos años, y destacaría especialmente su alto nivel de profesionalismo y enfoque riguroso para entregar soluciones tecnológicas para operaciones. Siempre está atento a las nuevas tecnologías y cómo aplicarlas a casos de uso del mundo real.

    Joaquín se destaca por su capacidad para diseñar e implementar sistemas robustos y escalables alineados con las necesidades del negocio. Es una persona confiable y colaborativa, lo que hace que trabajar con él sea consistentemente una experiencia muy positiva. Lo recomiendo sin dudarlo."


-   :material-format-quote-open:{ .lg .middle } Galbusera Sebastian

    Co-Fundador y Chief Product Officer en Plataforma PUMA

    ---

    "Joaquín demuestra excelente aptitud para desarrollar soluciones innovadoras, es proactivo en la investigación y procesamiento de información, diseña herramientas de comunicación técnica efectivas y trabaja excepcionalmente bien en entornos colaborativos."


-   :material-format-quote-open:{ .lg .middle } Francisco Lomazzi

    Gerente de Sustentabilidad en Espartina S.A.

    ---

    "Joaquín es alguien que está constantemente buscando mejorar las cosas que hace — no se conforma con que simplemente funcionen. Quiero destacar especialmente su creatividad, esfuerzo y dedicación, que lo impulsan a buscar la excelencia continuamente.

    Hemos trabajado juntos en varios proyectos relacionados con sustentabilidad, pero me gustaría destacar uno en particular: creó una automatización para evaluar el cumplimiento legal de los campos respecto a los requisitos de distancia de aplicación de agroquímicos. Fue mucho trabajo, pero lo entregó exitosamente.

    ¡Un verdadero placer trabajar con Joaquín!"


-   :material-format-quote-open:{ .lg .middle } Javier Moreira de Souza

    Especialista en Agricultura Digital | Consultor Agrícola.

    ---

    "Una persona intelectualmente aguda, mucha adrenalina al trabajar juntos, muy profesional y agradable, fue un placer haber trabajado juntos."


</div>

## Preguntas frecuentes

??? note "¿Qué tan rápido podés empezar a trabajar en mi proyecto?"
    Típicamente puedo comenzar nuevos proyectos dentro de 1-2 semanas de la firma del contrato. Para asuntos urgentes, mantengo cierta flexibilidad para situaciones de respuesta rápida y potencialmente puedo comenzar antes — solo contame tu timeline durante nuestra consulta inicial.

??? note "¿Requerís un tamaño mínimo de proyecto o compromiso?"
    Aunque puedo acomodar proyectos de cualquier tamaño, encuentro que los compromisos de al menos 20 horas permiten un impacto significativo. Esto nos da tiempo suficiente para entender tus datos, implementar soluciones y entregar resultados accionables. Podemos empezar con un pequeño proyecto piloto para asegurarnos de que somos un buen fit.

??? note "¿En qué industrias tenés experiencia?"
    He entregado proyectos exitosamente en operaciones agrícolas, administraciones, valuaciones de campos y servicios de seguros paramétricos. Aunque me especializo en IA, geoespacial y teledetección, también aplico mi conocimiento de IA y Python en proyectos que involucran todas las áreas de la empresa.

??? note "¿Cómo manejás la seguridad y confidencialidad de los datos?"
    Me tomo la seguridad de datos extremadamente en serio. Firmo NDAs comprensivos antes de comenzar cualquier proyecto, uso encriptación de nivel empresarial para todas las transferencias de datos y sigo las mejores prácticas de la industria para el manejo de datos. También puedo trabajar dentro de tu infraestructura y políticas de seguridad existentes.

??? note "¿Cuál es tu estructura de precios?"
    Ofrezco modelos de precios tanto por proyecto como por retainer. Las tarifas de proyecto se basan en alcance, complejidad y valor entregado en lugar de horas trabajadas. Para soporte continuo, ofrezco paquetes de retainer flexibles, y también puedo proporcionar soporte continuo por hora cuando eso se ajusta mejor a tus necesidades. Discutamos tus requerimientos específicos durante nuestra consulta para determinar el enfoque más rentable.

??? note "¿Cómo comunicás el progreso y los resultados?"
    Mantengo una comunicación clara a través de actualizaciones semanales de progreso y reuniones regulares de seguimiento. Vas a recibir documentación detallada de todos los análisis, hallazgos y recomendaciones.


<div class="grid cards" style="margin-top: 3rem" markdown>

-   :material-coffee:{ .lg .middle } ¡Tomemos un café virtual juntos!

    ---

    ¿Querés ver si somos un buen match? Charlemos y descubrámoslo. Agendá una sesión de estrategia gratuita de 30 minutos para discutir tus desafíos y explorar cómo podemos trabajar juntos.

    [Agendá una llamada gratuita :material-arrow-top-right:](https://calendly.com/joaquin-urruti/consultation-30min){ .md-button .md-button--primary }

</div>
